...
export default function NotificationsPage() {
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
            <div className="card-elevated flex flex-col gap-4 border border-white/70 dark:border-slate-700/70 bg-white/92 dark:bg-slate-800/90 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                        <Bell size={20} />
                    </div>
                    <div>
                        <p className="section-title text-accent-600 mb-1">Operational inbox</p>
                        <h1 className="text-2xl font-bold text-navy-900 dark:text-white leading-tight">Notifications</h1>
                        <p className="text-sm text-slate-500 dark:text-slate-400">{unreadCount} unread message{unreadCount !== 1 ? 's' : ''} across the workflows you currently use.</p>
                    </div>
                </div>
                {unreadCount > 0 && <button onClick={() => void markAllRead()} className="btn-secondary text-sm h-9"><Check size={14} /> Mark all read</button>}
            </div>

            <div className="card-elevated border border-gray-100 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-slate-800 overflow-hidden">
