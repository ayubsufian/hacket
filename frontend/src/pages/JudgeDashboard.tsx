...
export default function JudgeDashboard() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            getCriteria(selectedEvent).catch(() => [] as JudgingCriteria[]),
        ]).then(([subs, crits]) => {
            if (!active) return
            setSubmissions(subs.data)
            setCriteria(crits)
            if (crits.length > 0) setScoreForm(f => ({ ...f, criteriaId: crits[0].id }))
        ...
        <div className="space-y-6 max-w-6xl mx-auto animate-fade-up">
            ...
            <div className="card-elevated flex flex-col gap-4 border border-white/70 bg-white/92 p-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Review submissions, inspect score breakdowns, and submit evaluations per criteria.</p>
                </div>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="card-elevated flex flex-col h-[calc(100vh-260px)] border border-gray-100">
                                ...
                                : submissions.map(sub => {
                                    ...
                                    return (
                                        <button key={sub.id} type="button" onClick={() => void selectSub(sub)}
                                            className={`w-full group px-5 py-4 flex items-center justify-between text-left transition-all ${isSelected ? 'bg-indigo-50/50 border-l-4 border-l-indigo-500' : 'hover:bg-gray-50 border-l-4 border-l-transparent'}`}>
                                            <div className="min-w-0">
                                                <p className={`font-semibold line-clamp-1 ${isSelected ? 'text-indigo-900' : 'text-gray-900 group-hover:text-indigo-600'}`}>{sub.title}</p>
                                                <div className="flex items-center gap-2 mt-1.5">
                                                    <span className={`badge ${sub.status === 'SUBMITTED' ? 'badge-blue' : sub.status === 'SCORED' ? 'badge-green' : 'badge-gray'}`}>{sub.status}</span>
                                                    {sub.team?.name && <span className="text-xs text-gray-400 truncate">{sub.team.name}</span>}
                                                </div>
                                            </div>
                                            <ChevronRight size={18} className={`shrink-0 ml-2 ${isSelected ? 'text-indigo-500' : 'text-gray-300'}`} />
                                        </button>
                                    )
                                })
                        }
                ...
                <div className="lg:col-span-2 space-y-6 h-[calc(100vh-260px)] overflow-y-auto pr-2">
                    {selectedSub ? (
                            ...
                            <div className="card-elevated p-6 border border-gray-100">
                                <div className="flex items-start justify-between gap-4 flex-wrap">
                                    <div className="flex-1 min-w-0">
                                        <h2 className="text-xl font-bold text-gray-900 truncate">{selectedSub.title}</h2>
                                        ...
                                        {selectedSub.description && (
                                            <p className="mt-3 text-sm leading-relaxed text-gray-600 whitespace-pre-wrap line-clamp-4">{selectedSub.description}</p>
                                        )}
                                    </div>
                                    {allScored && (
                            ...
                            {breakdown.length > 0 && (
                                <div className="card-elevated overflow-hidden border border-gray-100">
                                    <div className="border-b border-gray-100 bg-gray-50/50 p-4">
                                        <span className="text-sm font-bold text-gray-900 uppercase tracking-wider">My Scores</span>
                                    </div>
                                    <div className="divide-y divide-gray-100">
                                        {breakdown.map((b, i) => (
                                            <div key={i} className="px-5 py-3">
                                                <div className="flex items-center justify-between mb-1.5">
                                                    <span className="text-sm font-semibold text-gray-900">{b.criteriaName}</span>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm font-bold text-indigo-600">{b.value}</span>
                                                        <span className="text-xs text-gray-400">/ {criteria.find(c => c.id === b.criteriaId)?.maxScore ?? 10} pts</span>
                                                ...
                                                {b.comment && <p className="mt-1.5 text-xs text-gray-500 italic">{b.comment}</p>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="flex h-full flex-col items-center justify-center text-gray-400 bg-white/50 rounded-2xl border border-dashed border-gray-200">
                            ...
                            <p className="text-sm text-gray-400 mt-1">Criteria will load automatically</p>
                        </div>
                    )}
                </div>
            </div>
