...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const topbar = (
    <header className={`sticky top-0 z-40 transition-all duration-300 ${isLanding
      ? 'bg-[linear-gradient(180deg,rgba(243,251,248,0.92)_0%,rgba(242,251,249,0.80)_100%)] dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.95)_0%,rgba(30,41,59,0.90)_100%)] backdrop-blur-xl border-b border-emerald-100/60 dark:border-slate-700/60 shadow-sm'
      : 'bg-white/92 dark:bg-slate-900/92 backdrop-blur-xl border-b border-white/70 dark:border-slate-700/70 shadow-sm'
      }`}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `rounded-xl px-4 py-2 text-sm font-medium transition-all duration-200 ${isActive
                  ...
                  : 'text-gray-500 dark:text-gray-400 hover:text-navy-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-slate-800'
                }`
              }
            >
              {n.label}
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={toggleTheme}
            className="flex items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-600 bg-white/95 dark:bg-slate-800 p-2 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all duration-200 shadow-sm"
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-2xl border border-slate-200 dark:border-slate-600 bg-white/95 dark:bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-all duration-200 shadow-sm"
          >
            <Globe size={13} />
            {locale === 'en' ? 'አማ' : 'EN'}
          </button>
          ...
          {isAuthenticated ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 rounded-2xl border border-slate-200 dark:border-slate-600 bg-white/95 dark:bg-slate-800 p-1 pr-3 text-sm shadow-sm transition-all duration-200 hover:shadow-card hover:border-slate-300 dark:hover:border-slate-500"
              >
              ...
              {userOpen && (
                <>
                  ...
                  <div className="absolute right-0 z-40 mt-2 w-60 rounded-[1.4rem] border border-white/80 dark:border-slate-700 bg-white/95 dark:bg-slate-900 py-1.5 shadow-elevated animate-scale-in origin-top-right backdrop-blur-xl">
          ...
          <button
            ...
            className="sm:hidden rounded-xl p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
          >
    ...
    return (
      <div className="flex h-screen bg-page dark:bg-page-dark">
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1.5">
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-2.5 rounded-2xl px-3 py-2.5 text-sm transition-all duration-200 ${isActive
                      ? 'bg-accent-50 dark:bg-accent-900/30 text-accent-700 dark:text-accent-400 font-semibold shadow-sm'
                      : 'text-slate-500 dark:text-slate-400 hover:text-navy-800 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800'
                    }`
                  }
                >
            ...
            {extraLinks.length > 0 && (
              <>
                ...
                <p className="mb-1 px-3 text-2xs font-semibold uppercase tracking-[0.18em] text-slate-300 dark:text-slate-500">Operations</p>
                {extraLinks.map(n => {
                  const Icon = n.icon
                  return (
                    <NavLink
          ...
          <div className="border-t border-slate-100 dark:border-slate-700 p-3">
            <div className="flex items-center gap-2.5 rounded-2xl p-2 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800">
              ...
              <button
                type="button"
                onClick={() => void handleLogout()}
                className="rounded-lg p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 transition-all duration-200"
              >
                <LogOut size={14} />
              </button>
            </div>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          {/* Top bar */}
          <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/70 dark:border-slate-700 bg-white/88 dark:bg-slate-900/88 px-4 backdrop-blur-xl lg:px-6">
            <button
              type="button"
              onClick={() => setMobileOpen(!mobileOpen)}
              className="lg:hidden rounded-xl p-1.5 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
            >
              <Menu size={18} />
            </button>
          ...
          <main className="flex-1 overflow-y-auto p-4 lg:p-6 bg-page dark:bg-page-dark">{children}</main>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-page dark:bg-page-dark">
      {topbar}
      <main>{children}</main>
      <footer className="border-t border-white/70 dark:border-slate-700 bg-white/88 dark:bg-slate-900/88 backdrop-blur-xl">
        <div className="mx-auto max-w-6xl px-4 py-12">
          <div className="mb-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {/* Links */}
            <div>
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Platform</p>
